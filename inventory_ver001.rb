...
class InventoryList  # item list within inventory file
  ...
  def addproduct
    # add new product into product_inventory array (multi dimensional??)
    #
    #  @product_inventory <<  ??
  end
  ...
  def get_next_available
    ...
    @shelf_array.each_with_index do |rack, row|
      ...
      if index != nil # We found an available spot
        rack_x = row
        shelf_y = index
        # @shelf_array[row][index] = "big hammer"
        break
      end
    end
    return {
...
class Rack
  def initialize
     @rack_array = ["small cups", nil, nil, nil, nil]
  end
...
while open_menu  # loop the ordering menu until no more customers
  ...
  if menu_select == 'X'
    open_menu = false
  else menu_select == 3  # option 3 is to add new products
    ...
    # To add a new product to product_inventory array
    # 1. get the next empy rack_x and shelf_y



    # 2 get next shelf
    inventory_list = InventoryList.new() # create a new inventory instance
